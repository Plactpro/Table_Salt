...
| F-122 | Medium | Money | `server/routers/restaurant-billing.ts` | 445-466 | Payment-time tax validation uses client-tampered bill.subtotal as basis — cannot catch a tampered bill | Open (reframed twice; see 00-backlog F-284 — bug is API returns 200 OK on payment requests against already-paid bills with no DB persistence; severity HIGH not CRITICAL) |
