...
- **F-257 [NEW 2026-05-05]: Currency setting does not propagate to Wastage Control or Shift Reconciliation reports.** Tester report 2026-05-05: tenant currency set to South Korean Won (KRW) in Settings, but Wastage Control page header shows "Today's Waste Cost AED0.00" and "Min Cost (AED)" filter — clearly hardcoded or reading from the wrong source. Same pattern in Shift Reconciliation. Two possible root causes: (a) shared with F-256 — write succeeds, read-back broken everywhere consistently, OR (b) Wastage Control and Shift Reconciliation have their own currency lookup path that bypasses tenant config. Verification needed from testers: was AED the correct currency for that tenant before the KRW change? If yes → cause (a). If no → cause (b). Severity: BLOCKING — any tenant in a non-AED currency will see incorrect currency labels on financial reports, which is regulatory and bookkeeping liability. Source: tester report 2026-05-05. Investigation: grep for hardcoded "AED" in client/src/, especially in `wastage*` and `shift*` modules.
