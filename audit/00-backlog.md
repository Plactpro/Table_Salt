...
2. **PR B — auto-create `delivery_orders` row in `POST /api/orders` for delivery-shaped order types.** Blocks launch: without it, every new POS-Delivery order placed in production becomes a fresh 404-orphan, defeating any backfill cleanup before testers even start. Best done after #1 lands clean so we know we're shipping onto a verified-stable baseline rather than stacking writes on top of unverified fixes.
3. **Production run of `scripts/backfill-delivery-orders-from-pos.ts`.** Affects end-to-end testing (testers can't verify Assign Agent on the 18 existing POS-Delivery orphans until this runs) and carries production-data-write risk (encrypts and inserts 18 rows across 2 tenants). Best done last: PR B in place first so the cleanup isn't immediately reversed by new orphans, and #1 verified first so we're not writing to production while a prior fix is still unconfirmed.
