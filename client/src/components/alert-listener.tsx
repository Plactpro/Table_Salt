...
const AlertListener: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (errorPrinters.length === 0) return;

    // Show the printer error toast only once per browser session.
    if (sessionStorage.getItem("printer_alert_shown")) return;
    sessionStorage.setItem("printer_alert_shown", "true");
