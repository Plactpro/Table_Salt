...
export function registerKitchenAssignmentRoutes(app: Express): void {
  ...
  app.get("/api/assignments/board", requireAuth, async (req, res) => {
    try {
      ...
      console.log("[KDS-DEBUG] tenantId=" + user.tenantId + " outletId=" + outletId + " rows=" + rows.length + " assigns0=" + (rows[0]?.assignments?.length ?? 0) + " totalCalc=" + rows.reduce((s,r) => s + (r.assignments?.length ?? 0), 0));
